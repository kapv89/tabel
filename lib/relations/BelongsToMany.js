...
var BelongsToMany = (function (_Relation) {
  ...
  function BelongsToMany(fromTable, toTable, pivotTable, foreignKey, otherKey) {
    var _this = this;

    _classCallCheck(this, BelongsToMany);

    _get(Object.getPrototypeOf(BelongsToMany.prototype), 'constructor', this).call(this);
    (0, _lodash.assign)(this, { fromTable: fromTable, toTable: toTable, pivotTable: pivotTable, foreignKey: foreignKey, otherKey: otherKey });

    this.pivotFields = [foreignKey, otherKey];

    this.constrain(function (t) {
      t.scope(function (q) {
        return q.join(_this.pivotTable.tableName(), _this.pivotTable.c(_this.otherKey), '=', toTable.key());
      });
    });
  }
  ...
  _createClass(BelongsToMany, [{
    ...
    value: function sync() {
      ...
      return pivotTable.fork().where(foreignKey, fromModel[fromTable.key()]).del().then(function () {
        ...
        var relatedKeys = relatedModels.map(function (m) {
          return (0, _lodash.isPlainObject)(m) ? m[toTable.key()] : m;
        });
    ...
    value: function join(tableContext) {
      ...
      if (tableContext.hasJoint(label)) {
        ...
        return this.joinPivot(tableContext).joint(function (q) {
    ...
    value: function leftJoin(tableContext) {
      ...
      if (tableContext.hasJoint(label)) {
        ...
        return this.leftJoinPivot(tableContext).joint(function (q) {
    ...
    key: 'leftJoinPivot',
    value: function leftJoinPivot(tableContext) {
      var joiner = arguments.length <= 1 || arguments[1] === undefined ? function () {} : arguments[1];
      var label = arguments.length <= 2 || arguments[2] === undefined ? null : arguments[2];
