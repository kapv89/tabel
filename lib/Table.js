...
var Table = function () {
  ...
  _createClass(Table, [{
    ...
    value: function fetchResultsFromCacheOrDatabase(q) {
      ...
      if (q._orm.cacheEnabled) {
        var _ret11 = function () {
          var key = _this27.queryCacheKey(q);

          return {
            v: _this27.cache.get(key).then(function (result) {
              if (result !== null) {
                return result;
              } else {
                return q.then(function (result) {
                  return _this27.cache(key, result, q._orm.cacheLifetime).then(function () {
                    return result;
                  });
                });
              }
    ...
    value: function processResult(result, options) {
      ...
      if (count === true) {
        ...
        return result.map(function (row) {
          return _this28.processResult(row, { count: count });
        });
        ...
        return Object.keys(result).reduce(function (processed, key) {
          if (key.indexOf('.') > -1) {
            if (key.indexOf(_this28.tableName()) === 0) {
    ...
    value: function loadRelations(models, eagerLoads) {
      ...
      .filter(function (relation) {
        ...
        if (!_this29.definedRelations.has(relation)) {
          throw new Error('invalid relation ' + relation);
        }

        return _this29[relation]().eagerLoad(_this29.subEagerLoads(relation, eagerLoads)).constrain(eagerLoads[relation]).load(models);
    ...
    value: function all() {
      ...
      return this.fetchResultsFromCacheOrDatabase(q).then(function (models) {
        return _this30.processResult(models);
      }).then(function (models) {
        return _this30.loadRelations(models, q._orm.eagerLoads);
      }).then(function (models) {
    ...
    value: function count() {
      ...
      var q = this.attachOrmNSToQuery(this.orm.knex.count('*').from(function (q) {
        q.from(_this31.tableName());

        _this31.scopeTrack.apply(q);

        if (!_this31.scopeTrack.hasScope('select')) {
          q.select(_this31.c('*'));
        }
      ...
      return this.fetchResultsFromCacheOrDatabase(q).then(function (result) {
        return _this31.processResult(result, { count: true });
      });
    }
    ...
    value: function orderedFork() {
      ...
      var labels = this.scopeTrack.scopes.map(function (_ref11) {
    ...
    value: function batchReduce() {
      ...
      var reduceBatchN = function reduceBatchN(batchNum, batchInitialVal) {
        return _this33.orderedFork().limit(batchSize).offset((batchNum - 1) * batchSize).all().then(function (models) {
          try {
    ...
    value: function reduce() {
      var reducer = arguments.length <= 0 || arguments[0] === undefined ? function () {} : arguments[0];
      var initialVal = arguments.length <= 1 || arguments[1] === undefined ? null : arguments[1];
    ...
    key: 'map',
    value: function map() {
      var _map = arguments.length <= 0 || arguments[0] === undefined ? function () {} : arguments[0];
    ...
    value: function find() {
      ...
      switch (arguments.length) {
        ...
        case 1:
          return function (val) {
            if (isUsableObject(val)) {
              return _this34.where(val).first();
            } else {
              return _this34.whereKey(val).first();
            }
        ...
        case 1:
          return function (condition) {
            if (isUsableObject(condition)) {
              return _this35.where(condition).del();
            } else {
              // else we are deleting based on a key
              return _this35.whereKey(condition).del();
            }
          }.apply(undefined, arguments);
        default:
    ...
    value: function attachTimestampToValues(values, _ref12) {
      var _this36 = this;
      ...
      if (isArray(values)) {
        return values.map(function (val) {
          return _this36.attachTimestampToValues(val);
        });
      }
    ...
    value: function pickColumnValues(columns, values, _ref13) {
      var _this38 = this;
      ...
      if (isArray(values)) {
        return values.map(function (v) {
          return _this38.pickColumnValues(columns, v);
        });
      }
    ...
    value: function insertModel(model) {
      ...
      return Promise.all([this.columns(), this.genKeyVal()]).then(function (_ref14) {
        var _ref15 = _slicedToArray(_ref14, 2);

        var columns = _ref15[0];
        var keyVal = _ref15[1];

        model = assign({}, keyVal, _this39.attachTimestampToValues(_this39.pickColumnValues(columns, model, opFlags), opFlags));

        return _this39.newQuery().returning('*').insert(model).then(function (_ref16) {
    ...
    key: 'rawUpdate',
    value: function rawUpdate(values, _ref18) {
      var _this41 = this;

      var _ref18$returning = _ref18.returning;
      var returning = _ref18$returning === undefined ? false : _ref18$returning;

      var opFlags = { op: 'update' };

      return this.columns().then(function (columns) {
        values = _this41.attachTimestampToValues(_this41.pickColumnValues(columns, values, opFlags), opFlags);
        ...
        if (returning === true) {
          ...
          return _this41.query().returning('*').update(values).then(function (_ref19) {
          ...
          // just use knex!
          return _this41.query().update(values);
        }
      });
    }
