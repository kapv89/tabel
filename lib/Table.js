  ...
  _createClass(Table, [{
    ...
    value: function whereIn(field) {
      ...
      if (vals.length === 0) {
        ...
        if (isArray(field)) {
          return this.whereRaw('(' + this.c(field).join(',') + ') in (' + vals.map(function () {
            return '(' + field.map(function () {
              return '?';
            }).join(',') + ')';
          }).join(',') + ')', vals.map(function (v) {
    ...
    value: function orWhereIn(field) {
      ...
      if (vals.length === 0) {
        ...
        if (isArray(field)) {
          return this.orWhereRaw('(' + this.c(field).join(',') + ') in (' + vals.map(function () {
            return '(' + field.map(function () {
              return '?';
            }).join(',') + ')';
          }).join(',') + ')', vals.map(function (v) {
    ...
    value: function whereNotIn(field) {
      ...
      if (vals.length === 0) {
        ...
        if (isArray(field)) {
          return this.whereRaw('(' + this.c(field).join(',') + ') not in (' + vals.map(function () {
            return '(' + field.map(function () {
              return '?';
            }).join(',') + ')';
          }).join(',') + ')', vals.map(function (v) {
    ...
    value: function orWhereNotIn(field) {
      ...
      if (vals.length === 0) {
        ...
        if (isArray(field)) {
          return this.orWhereRaw('(' + this.c(field).join(',') + ') not in (' + vals.map(function () {
            return '(' + field.map(function () {
              return '?';
            }).join(',') + ')';
          }).join(',') + ')', vals.map(function (v) {
    ...
    value: function insertModel(model) {
      ...
      return Promise.all([this.columns(), this.genKeyVal()]).then(function (_ref15) {
        ...
        if (isPostgres(_this36.orm)) {
          ...
          if (_this36.props.increments) {
            return _this36.newQuery().insert(model).then(function (_ref19) {
    ...
    key: 'rawUpdate',
    value: function rawUpdate(values, _ref21) {
      var _this38 = this;

      var _ref21$returning = _ref21.returning,
          returning = _ref21$returning === undefined ? false : _ref21$returning;
      ...
      return this.columns().then(function (columns) {
        ...
        if (returning === true) {
          ...
          if (isPostgres(_this38.orm)) {
            return _this38.query().returning('*').update(values).then(function (_ref22) {
            ...
            return _this38.fork().first().then(function (model) {
              return _this38.query().update(values).then(function () {
                return _extends({}, model, values);
              });
            });
          } else {
            throw new UnsupportedDbError();
