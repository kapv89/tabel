...
var Table = function () {
  ...
  _createClass(Table, [{
    ...
    value: function all() {
      ...
      return this.uncacheQueryIfNeeded(q).then(function () {
        ...
        return models.map(function (m) {
          return q._orm.maps.length === 0 ? m : q._orm.maps.reduce(function (m, map) {
            return map(m);
          }, m);
        });
      });
    ...
    value: function reduce() {
      ...
      return this.orderByRaw('(' + (isArray(this.key()) ? this.key() : [this.key()]).map(function (c) {
        return _this32.c(c) + '::text';
      }).join(' || ') + ') desc').batchReduce(500, function (batchInitialVal, models) {
    ...
    value: function find() {
      ...
      switch (arguments.length) {
        ...
        case 1:
          return function (val) {
            if (isUsableObject(val)) {
              return _this33.where(val).first();
            } else {
              return _this33.whereKey(val).first();
            }
        ...
        case 1:
          return function (condition) {
            if (isUsableObject(condition)) {
              return _this34.where(condition).del();
            } else {
              // else we are deleting based on a key
              return _this34.whereKey(condition).del();
            }
          }.apply(undefined, arguments);
        default:
    ...
    value: function attachTimestampToValues(values, _ref11) {
      ...
      if (isArray(values)) {
        return values.map(function (val) {
          return _this35.attachTimestampToValues(val);
        });
      }
    ...
    value: function insertModel(model) {
      ...
      return Promise.all([this.columns(), this.genKeyVal()]).then(function (_ref13) {
        ...
        return _this38.newQuery().returning('*').insert(model).then(function (_ref15) {
    ...
    value: function update() {
      ...
      if (args.length >= 2) {
        ...
        return this.whereKey(keyCondition).rawUpdate(values, { returning: true, keyCondition: keyCondition });
    ...
    value: function rawUpdate(values, _ref17) {
      ...
      return this.columns().then(function (columns) {
        ...
        if (returning === true) {
          ...
          return _this40.query().returning('*').update(values).then(function (_ref18) {
